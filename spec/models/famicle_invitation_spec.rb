...
describe FamicleInvitation do
  ...
  describe "inviting a member" do
    ...
    it "should only be allowed by an owner" do
      @other_user = create_user({:email => "invited@example.com"})
  ...
  describe "handling more than one invite for a non user" do
    before(:each) do
      @creator = create_user({:email => "creator@example.com"})
      @famicle = create_famicle(@creator)
      @another_creator = create_user({:email => "creator2@example.com"})
      ...
      @new_user = create_user(:email => "nonmember@example.com")
      @new_user.activate!
    end
