...
describe SessionsController do
  ...
  before do 
    ...
    User.stubs(:authenticate).with(@login_params[:email], @login_params[:password]).returns(@user)
  end
  def do_create
          ...
          describe "and ask #{want_remember_me ? 'to' : 'not to'} be remembered" do 
            ...
            if (has_request_token == :valid)
              ...
              if want_remember_me
                ...
                it "does not refresh token"  do @user.expects(:refresh_token).never; do_create end
                it "sets an auth cookie"       do do_create;  end
              else 
                it 'does not make new token' do @user.expects(:remember_me).never;   do_create end
                it 'does not refresh token'  do @user.expects(:refresh_token).never; do_create end
